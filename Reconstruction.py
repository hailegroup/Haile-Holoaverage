...
class HoloReconstructor(object):
    ...
    #    _destShape   Shape of destination data
    ...
    def bpf_correct(self, data):
        ...
        tmp = tmp[sy0:sy1, sx0:sx1]
        rmax = int(np.ceil(max((self._mask!=0).sum(0)) / 2) * 1.2)
        roi = [tmp.shape[0] // 2 - rmax, tmp.shape[0] // 2 + rmax,
               tmp.shape[1] // 2 - rmax, tmp.shape[1] // 2 + rmax]
        tmp = np.log(abs(tmp) + 1)
        self.bpfilter = LineFilter(tmp, roi=roi)
    ...
    def apply(self, data):
        ...
        if self.bpfilter is not None:
            ravg = self.bpfilter.rotate_average(tmp, self._mask)
            tmp = np.multiply(tmp, self.bpfilter.pts) + ravg  
        
        side[dy0:dy1, dx0:dx1] = tmp * self._mask
...
def holo_reconstruction(data, carrier=None, qMax=None, qMax2=None, shape=None, mtf=None, maskType=None, bpf=False):
    """
    ...
    Arguments:
        ...
        bpf
            Flag for applying biprism Fresnel fringe correction, or
            instance of the LineFilter class
    """
    ...
    if bpf==True:
        reconstructor.bpf_correct(data)
    elif isinstance(bpf, LineFilter):
        reconstructor.bpfilter = bpf
    return reconstructor(data), reconstructor.bpfilter


def series_reconstruction(series, carrier=None, qMax=None, qMax2=None, shape=None, mtf=None, verbose=0, maskType=None, bpf=False):
    ...
    reconstructor = HoloReconstructor(grid, carrier=carrier, shape=shape, mask=maskType, mtf=mtf)
    if bpf==True:
        reconstructor.bpf_correct(series[(0,)])
    elif isinstance(bpf, LineFilter):
        reconstructor.bpfilter = bpf
    ...
    return destSeries, reconstructor.bpfilter
