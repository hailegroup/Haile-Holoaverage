...
def rawAlign(series, qMax=None, verbose=0, roi=None, filter=None, bpf=False):
    ...
    Arguments:
        ...
        bpf
            Flag to correct biprism Fresnel fringes
    ...
    thisFFT = grid.forwardFFT(thisFFT) #FFT of first image
    
    if bpf:
        mask_radius = max((mask!=0).sum(0)) // 2
        bpf_roi = (thisFFT.shape[0] // 2 - int(1.2 * mask_radius), 
               thisFFT.shape[0] // 2 + int(1.2 * mask_radius))
        bpf_roi2 = [bpf_roi[0], bpf_roi[1], bpf_roi[0], bpf_roi[1]]
        #make roi 20% larger than mask for line filter correction
        tmp = np.fft.fftshift(thisFFT)
        tmp = np.log(abs(tmp) + 1)
        lf = LineFilter(tmp, bpf_roi2)        
        lf_pts = np.fft.ifftshift(lf.pts)
        ravg = np.fft.ifftshift(lf.rotate_average(np.fft.fftshift(thisFFT), np.fft.fftshift(mask)))
        thisFFT = np.multiply(thisFFT, lf_pts) + ravg
    ...
    for i in range(1, series.indexSize):
        ...
        thisFFT = grid.forwardFFT(thisFFT) #FFT of next image
        
        if bpf:
            ravg = np.fft.ifftshift(lf.rotate_average(np.fft.fftshift(thisFFT), np.fft.fftshift(mask)))
            thisFFT = np.multiply(thisFFT, lf_pts) + ravg
        scale = 0.5 * abs(thisFFT[0, 0]) + 0.5 * abs(lastFFT[0, 0]) #arbitrary constant
        tmp = lastFFT * np.conj(thisFFT)
        tmp /= (abs(tmp) + 0.0001 * scale) #scale added to avoid singularities
