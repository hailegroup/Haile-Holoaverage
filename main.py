...
def create_synthetic_empty(camera_dev, sideband_pos, empty_size, sampling, mask_type):
    ...
    empty_holo, empty_lf = holo_reconstruction(empty, carrier=carrier, shape=(empty_size, empty_size), maskType=mask_type)
    return empty_holo
...
def holoaverage(param, basepath="", verbose=0):
    ...
    bpf_correct = bool(param.get('bpf_correct', False))
    ...
    if synthesize_empty:
        ...
        empty_holo, empty_lf = series_reconstruction(empty_series, carrier=carrier, shape=(empty_size, empty_size), verbose=verbose, mtf=mtf, maskType=mask_type, bpf=bpf_correct)
        del empty_series
    ...
    if enable_raw_alignment and len(data_series) > 1:
        data_series = rawAlign(data_series, qMax=align_cut_off, verbose=verbose, roi=align_roi, bpf=bpf_correct)
    ...
    holo_series, holo_lf = series_reconstruction(data_rois, carrier=carrier, shape=(object_size, object_size), verbose=verbose, mtf=mtf, maskType=mask_type, bpf=empty_lf)
    del data_rois
...
def main(argv=None):
    ...
    """Connor Carr's addition: open Tkinter window to set parameters
    Import parameters from window without creating or decoding JSON file"""
    
    if param_file == 'tkinter':
        tk_flag = 1
        if verbose > 0:
            print("Loading parameters from Tkinter GUI")
        param = tk_param()
        basepath = ''
        # basepath and prefix paths aren't necessary in this case
